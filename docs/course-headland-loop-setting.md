...
streams retain the attribute. The generator setting is the sole working owner. Existing savegames
and implement profiles migrate the former vehicle-setting key when they are read; later saves write
only the generator key. Turn manoeuvres read the active course directly.
...
Test build 004 uses version `8.1.0.321`, filename `FS25_Courseplay_ImplementProfilesTest.zip` and
