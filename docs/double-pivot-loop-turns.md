# Multi-pivot headland loop turns

## Main integration - 8.1.0.28

Merged `codex/multi-pivot-headland-loop-turns` into main after the user confirmed
test 8.1.0.316 works as intended. Wide drills with trailing carts need each hitch,
body and working width checked: a tractor-only radius cannot represent cart
offtracking or drill/cart clearance. The planner searches forward loops within
the field and validates the complete chain before installing a route.

Planning yields between bounded steps to keep the game responsive. The return
follows the actual outgoing headland, including its bends, and retains a mapped
fieldwork waypoint so it does not stop looking for the old corner behind it.
Lowering uses the existing work-start line, and the full loop retains the user's
configured turn speed. Unsupported layouts retain their existing fallback.

The five modules below explain their responsibilities and constraints in code.
The refactor preserves the .315 routes and improves local benchmark time by
about 10-16%. This remains a planar approximation; it does not make the toolbar
centre follow the displayed tractor path. That experiment was not merged.

Merge resolution keeps main's straight-entry, plough and forager changes. Reuse
the completed loop search rather than running it a second time when installing
the turn. Keep main's normal title and version separate from the numbered test
identity: `tools/double-pivot/build_main_test.py` qualifies source and extracted
ZIP scripts, then produces test 8.1.0.317 with only its manifest rebranded.
