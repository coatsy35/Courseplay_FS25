...
--- Estimated seconds until this harvester reaches its configured normal unloader call level.
--- The measured harvest rate is preferred; the fallback deliberately remains conservative while the
--- first row is establishing a usable rate.
---@return number
function AIDriveStrategyCombineCourse:getSecondsUntilUnloaderCall()
    if self:alwaysNeedsUnloader() then
        return 0
    end
    local fillLevel = self.combineController:getFillLevel()
    local capacity = self.combineController:getCapacity()
    local targetFillLevel = capacity * self.settings.callUnloaderPercent:getValue() / 100
    local litresRemaining = math.max(0, targetFillLevel - fillLevel)
    if self.litersPerSecond and self.litersPerSecond > 0.1 then
        return litresRemaining / self.litersPerSecond
    end
    local percentageRemaining = math.max(0,
            self.settings.callUnloaderPercent:getValue() - self.combineController:getFillLevelPercentage())
...
function AIDriveStrategyCombineCourse:isAutoDriveWaitingForPipe()
    if self.vehicle.spec_autodrive then
        ...
        elseif self.vehicle.spec_autodrive.combineIsCallingDriver then
            ...
            return self.vehicle.spec_autodrive:combineIsCallingDriver(self.vehicle)
        end
    end
    return false
end
