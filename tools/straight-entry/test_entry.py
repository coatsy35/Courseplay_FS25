...
class EntryTests(unittest.TestCase):
                ...
                for _,side in ipairs({false,true}) do
                    for _,duration in ipairs({1,7,15}) do
                        for _,dt in ipairs({16,33,100}) do
                            ...
                            f.tool.animation=side and 1 or 0
    ...
    def test_turnover_blocks_lowering_even_when_marker_reaches_work_start(self):
        ...
        self.lua.execute('''
            ...
            f.tool.playing=false; f.tool.animation=f.tool.wanted and 1 or 0
        ...
        self.lua.execute("""
            for _,heading in ipairs({-179,-95,-45,45,95,179}) do
                for _,side in ipairs({-1,1}) do
                    ...
                    assert(f:drive()==0 and f.tool.rotateCount==1 and f.tool.lowerCount==0)
                    f.tool.animation=f.tool.wanted and 1 or 0; f.tool.playing=false
    ...
    def test_towed_turnover_waits_for_drawbar_alignment_without_stopping(self):
        ...
        for _, recovery in ipairs({false,true}) do
            ...
            assert(f.tool.wanted==(side<0))
            assert(f:drive()==0 and f.tool.rotateCount==1)
            f.tool.animation=side<0 and 1 or 0; f.tool.playing=false
            assert(f:drive()==speed and f.tool.lowerCount==1)
        end
        end
        end
    ...
    def test_wrong_working_side_cannot_lower_while_drawbar_is_still_bent(self):
        self.lua.execute((SOURCE / 'tools/straight-entry/preparation-fixture.lua').read_text())
        self.lua.execute("""
        for _,side in ipairs({false,true}) do
            local f=preparationFixture(20,side,true)
            f:position(-.1)
            f.tool.animation=side and 0 or 1 -- fully rotated, but wrong side
            f.tool.drawbarNode.t=math.rad(20)
            assert(f:drive()==20 and f.tool.rotateCount==0 and f.tool.lowerCount==0)
            f.tool.drawbarNode.t=0
            assert(f:drive()==0 and f.tool.rotateCount==1 and f.tool.lowerCount==0)
            f.tool.animation=side and 1 or 0; f.tool.playing=false
            assert(f:drive()==20 and f.tool.lowerCount==1)
        end
        """)
